...
- [x] F7 억제 사유 노출 (review-personas) — ✅ 2026-09-08 배포 (v20260908T080313)
  - `common/alert_verdict.py`(판정 최종값 + 사람 문구, 억제율 리포트와 **공유**), `GET /alert/events`,
    프런트 `/alerts` 화면(요약·필터·행 펼침 설명·격리 해제 시각·활성 정비창)
...
- [x] 2.1.1 `IncidentTable` — ✅ 2026-09-09 배포 (v20260909T053247)
  - PK `incident_id`, GSI `customer_id-index`(최신순), TTL 90일. 열린 사건 포인터는 상태 표의 `inc#{그룹 키}`
    (`{고객사 or 계정}#{등급}` — 그룹과 같은 축, review-phase2 M3). 모든 쓰기는 `version` 조건부(H2).
  - 병합 축은 그룹과 같지만(고객사×등급) **수명이 다르다** — 원인 알람이 모두 풀릴 때까지 한 사건
- [x] 2.1.2 생명주기 — ✅ 2026-09-09. 별도 상태 머신 없이 **라우터가 발송과 같은 자리에서** 연다/합친다/해소한다
  - 발송 직전이 사건을 아는 유일한 지점이라, 단계를 더하면 같은 데이터를 두 번 읽게 된다
  - [x] 테스트: 원인 알람 전부 OK → 자동 resolved (R4-4) / 하나라도 열려 있으면 유지
  - [x] 테스트: 확인은 한 번만 기록 — 두 번째가 시각을 덮으면 MTTA가 거짓이 된다 (R4-3)
  - [x] 테스트: 창이 여러 번 지나도 같은 사건에 합쳐진다
  - [x] 재알림 (R4-6) — ✅ 2026-09-09 배포 (v20260909T054653). **에스컬레이션과는 별개다** —
    에스컬레이션은 "다음 사람에게"라 온콜 축(U1)이 필요하지만, 재알림은 "같은 채널에 다시"라 막힌 게 없다
    - 5분마다 라우터를 깨워 확인 후 `renotify_after_sec`(정책, 기본 1시간) 넘긴 사건을 다시 띄운다
    - 보내기 **전에** 조건부로 표시한다 — 겹친 실행이 같은 사건을 두 번 울리지 않게
    - 보낼 채널이 없으면 표시하지 않는다(채널이 생기면 그때 울려야 한다)
    - [x] 테스트: 창 안에서는 조용 / 표시 후 다음 주기 조용 / 다음 창에서 다시 / 미확인·해소 건 제외
    - [x] 라이브: 2시간 방치 사건 → 슬랙 "[미해결]" 발송, 재실행 시 침묵, 해소 후 제외, 주기 룰 ENABLED
- [x] 2.1.3 ack 수신 경로 — ✅ `POST /alert/incidents/{id}/ack`. **확인자는 로그인 신원에서 온다**
  (본문으로 받으면 남을 대신 확인할 수 있다). 알림에 사건 ID와 링크가 실린다
- [x] 2.1.4 MTTA·MTTR 산출 (R4-5) — ✅ 사건별 값 + `GET /alert/incidents`의 집계
  - 라이브: 발화 → 확인(MTTA 8초) → 해소(MTTR 46초) → 집계까지 8개 항목 검증

### 2.2 채널

> **선행 결정 필요:** U2(등록 주체), U3(조건 범위) — design.md §5

> **확장 설계 확정 (2026-09-09): `design-notification-channels.md`.**
> 채널 유형 추가 = `notification_adapters.py`에 `Adapter(...)` 블록 하나. 검증·응답 자격증명
> 제거·유형 목록·속도 제한·계약 테스트가 그 선언에서 자동으로 따라온다.
> **U2 = 우리가 대행, U3 = 등급+리소스타입+계정**으로 진행.

- [x] 2.2.0 채널 코어 (순수) — ✅ 2026-09-09
  - `common/notification_adapters.py` 레지스트리 + Slack·이메일·범용 Webhook
  - `common/notification_channel.py` 채널 모델·검증·직렬화·조건 매칭
  - [x] 테스트 54건. **모든 어댑터 공통 계약 검사** 포함 — 새 유형은 테스트를 새로 안 써도 걸린다
  - [x] 테스트: 자격증명이 API 응답 표현에 나오지 않음 / 저장 표현에는 남음
  - [x] 테스트: 모르는 설정 키·모르는 조건 축은 저장 단계에서 거절
- [x] 2.2.1 `NotificationChannelTable` + CRUD API — ✅ 2026-09-09 배포 (v20260909T035645)
  - 표 PK `customer_id` / SK `channel_id`. **전역 채널은 `__global__` 센티넬로 저장** —
    DynamoDB는 키 속성에 빈 문자열을 못 넣는다(라이브에서 발견, 가짜 테이블이 제약을 흉내 내지 않아 놓쳤다)
  - `GET /alert/channel-types`(화면이 폼을 그릴 카탈로그), `GET/POST/PUT/DELETE /alert/channels`
  - 전역 채널 등록·수정·삭제는 관리자 전용(전역 정비창과 같은 폭발 반경)
  - [x] 테스트: 생성·목록·수정 **어느 응답에도 자격증명이 없다** / 저장에는 남는다
  - [x] 테스트: 수정 시 자격증명 미전송·가림 문자열 모두 "안 바꿈"으로 읽는다
  - [x] 테스트: 가짜 테이블이 빈 키를 실제처럼 거절한다
...
- [x] 2.2.3 라우터 Lambda — 조건 매칭 → 채널별 발송 — ✅ 2026-09-09 배포 (v20260909T045008)
  - **GSI는 최종 일관성** — finalize가 방금 쓴 `final_action`이 인덱스에 없어 발송이 통째로 누락됐다
    (라이브에서만 드러남: 상태 머신 SUCCEEDED인데 알림만 안 감). 인덱스로는 **키만** 찾고 상태는
    기본 테이블에서 `BatchGetItem`(ConsistentRead)으로 읽는다
  - [x] 테스트: 조건에 맞는 채널만 선택됨
  - [x] 테스트: 한 채널 실패가 다른 채널 발송을 막지 않음 / 인덱스 지연에도 발송된다
- [x] 2.2.4 어댑터 — Slack / Email(SES) / 범용 Webhook — ✅ 2026-09-09
  - 전송 방식도 어댑터가 선언한다(`transport`/`endpoint_field`/`recipients_field`) — 유형이 늘어도 전송 코드는 그대로
  - 이메일은 SES. `ALERT_EMAIL_SENDER` 미설정이면 **기록된 오류**로 남는다(조용히 죽지 않음)
- [x] 2.2.5 채널별 속도 제한 + 폭풍 시 묶음 발송 (R6-9) — ✅ 2026-09-09
  - 그룹 하나 = 채널당 **한 통**. 200건이면 "대표 1건 + 외 199건"(대표는 가장 심각한 것)
  - 같은 유형끼리 어댑터 선언 속도만큼 간격. **실행 간** 상한은 그룹핑이 대신 잡는다(창당 1건) —
    부족해지면 공유 토큰 버킷이 필요하다
  - [x] 테스트: Slack 초당 1건 간격, 200건 → 한 통 + "외 199건"
- [x] 2.2.6 재시도 + 최종 실패 기록 (R6-7) — ✅ 2026-09-09
...
- [ ] 4a.4 **노이즈 스코어** — 단일 축 flapping 판정을 다축 점수로 일반화
  > **왜:** `is_flapping()`은 이미 점수(발화횟수/일) ≥ 임계값 형태다. 축이 하나뿐이라
  > "하루 2번 발화하지만 매번 30초 만에 자동 복구되는 알람"을 잡지 못한다. 억제 규칙을
  > 더 쌓는 대신 축을 늘린 점수 하나로 합친다. **모델을 쓰지 않는다** — 결정론적 계산이라
  > 같은 입력에 같은 결과가 나오고, 사유를 사람에게 설명할 수 있다.
  > (외부 확률 분류 모델을 검토했으나 채택하지 않았다: 라벨 데이터가 없고, 놓친 장애의
  > 비용이 노이즈 한 건보다 압도적으로 크며, 추론 인프라가 비용 베이스라인을 넘긴다.)
  - [ ] 4a.4.1 축 정의 — `EventHistoryTable` 기반 (4a.1.4 선행)
    - 발화 빈도 (일평균) / 자동 복구율 (사람 개입 없이 해소된 비율) / 평균 ALARM 지속시간
    - **알람 이름을 파싱하지 않는다** — `series_id`와 이벤트 메타데이터만 쓴다 (AGENTS.md AP-3·AP-23)
  - [ ] 4a.4.2 `common/alert_suppression.py`에 순수 함수로 추가 — `noise_score(stats) -> float`
    - `decide()`의 평가 순서에 3.5단계로 끼운다 (flapping 격리 다음, 중복 판정 앞)
    - 새 사유 상수 `REASON_NOISE_SCORE` 추가 — `EventHistoryTable`에 기록되고
      `scripts/alert_suppression_report.py`(R9-2)의 집계 축이 된다
  - [ ] 4a.4.3 **안전 장치 — 이 항목 없이는 병합하지 않는다**
    - SEV-1 면제(R3-8)를 우회하지 않는다
    - 점수는 **DEFER(유예 연장)까지만** 할 수 있고 NOTIFY를 SUPPRESS로 바꾸지 않는다.
      놓친 장애 비용이 노이즈 비용보다 크므로 기본 방향은 "늦게 보내기"다
    - 임계값은 `SuppressionPolicy`에 두고 기본값은 **비워둔다** — `auto_pause_sec`과 같은
      규약(값이 없으면 동작하지 않음). 실측 없이 켜지 않는다
  - [ ] 4a.4.4 백테스트 — `scripts/analyze_alarm_history.py`의 이력에 소급 적용
    - "이 점수로 유예됐을 알림" 중 대리 지표(해소까지 지속시간, 사람 ack 여부)로 본
      실제 인시던트 비율을 센다. 정답 라벨이 없으므로 대리 지표임을 리포트에 명시한다
    - 선행: **Phase 0.1.7 실계정 실행** — 실측 없이는 축의 분포도 임계값도 정할 수 없다
