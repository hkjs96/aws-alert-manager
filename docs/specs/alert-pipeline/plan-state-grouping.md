...
| 단계 | 결과 |
|---|---|
| **A** | 25번 토글 → 발화 NOTIFY 1 · dedup 1 · flapping 23, 해소 알림 1, 상태 version 50, 충돌 0 ✅ |
| **B** | 알람 250개 × (ALARM, OK) 500건, 163초 유입 → **실행 6개**(30초 창마다 1개), 500건 전부 `group_id`·`final_action`, 실행 전부 SUCCEEDED, 오류·경합 0 ✅ |

**B에서 계획과 달라진 것 두 가지.**

1. **구성원 자격을 시간 창에서 `group_id`로 바꿨다.** 구현하면서 창 방식의 빈틈이 드러났다: 워커가
   조회 → 닫기 순이면 그 사이 도착한 이벤트가 고아가 되고, 닫기 → 조회 순으로 바꿔도 "닫힌 뒤 처리됐지만
   발생 시각은 닫기 전"인 늦은 이벤트가 새 창의 시작(`max(발생, 닫힘)`) 밖에 놓여 고아가 된다. 발생 시각과
   처리 시각이 다른 이상 창으로는 못 푼다. 인제스터가 적재 시 `group_id`를 적으면 이벤트는 정확히 한
   그룹에 속하고, 워커는 sparse GSI로 그 ID를 읽는다. 닫기 → Grace 5초 → 조회 순서는 유지한다.
2. **"1분 500건 → 실행 1개"는 라이브로 재현할 수 없었다.** CloudWatch `SetAlarmState`가 ~3 TPS로
   스로틀돼 500건이 163초에 걸쳐 들어왔고, group_wait 30초 창마다 그룹이 열려 실행 6개가 됐다. 이건
   설계대로다 — 실행 수가 **이벤트 수(500)가 아니라 시간 창 수(6)** 에 비례한다는 것이 라이브로 확인된
   사실이고, 타이트한 폭풍(500건 → 실행 1개)은 조건식 해석 가짜 테이블 + 가짜 Step Functions로 핸들러
   단위에서 고정했다.

**배포 주의.** 기존 테이블에 GSI를 추가하면 CFN은 백필 전에 완료를 반환한다(확인 시점 `CREATING`).
이번엔 실행이 돌 때 ACTIVE였지만, 큰 테이블이면 배포 직후 몇 분간 워커 조회가 실패할 수 있다.
