...
사건 `acknowledged` 유지·`version=2`·`resolved=0`. Slack 표시는 **09-11 12:45 KST 재발송 후 사용자가 확인** —
`< 10GB & > 5%`가 글자 그대로 보인다. 이스케이프 자체는 단위 테스트가 고정한다.
...
붙으면 자연히 밟힌다. 사용자 결정으로 남긴 것: `AlertEmailSender` — us-east-1 SES는 샌드박스(일 200통·초당 1통, 발신·수신 주소 모두 검증
필요, 검증된 주소 없음)이고, 이메일 유형은 "두 번째 채널 유형이 선언 하나로 붙는가"의 예시로 들어간 것이라 **실제 이메일
알림 수요가 생길 때** 검증하면 된다. 그때까지 카탈로그는 이메일을 비활성으로 보여 주며 아무것도 깨지지 않는다.
Slack의 `<`·`&`·`>` 표시는 사용자가 확인했다(09-11).
