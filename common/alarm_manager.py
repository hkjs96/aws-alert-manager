...
def _find_alarms_for_resource(
    ...
    1) 레거시: AlarmNamePrefix=resource_id
    2) 새 포맷: AlarmNamePrefix="[{resource_type}]" + suffix 필터
       resource_type 미지정 시 EC2/RDS/ELB 모두 검색
    """
    cw = _get_cw_client()
    ...
    prefixes = (
        [f"[{resource_type}] "]
        if resource_type
        else [f"[{rt}] " for rt in ("EC2", "RDS", "ELB")]
    )
    for prefix in prefixes:
        try:
            ...
            logger.error(
                "Failed to list new-format alarms prefix=%s for %s: %s",
...
def sync_alarms_for_resource(
    ...
    # 현재 알람 목록 조회
    existing_alarms = _find_alarms_for_resource(resource_id, resource_type)
