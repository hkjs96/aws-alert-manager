...
- [x] 1. `common/__init__.py` 상수 업데이트
...
- [x] 2. RDS Collector 메타데이터 enrichment (`common/collectors/rds.py`)
  - [x] 2.1 테스트 작성: `_enrich_aurora_metadata()` 및 `_INSTANCE_CLASS_MEMORY_MAP` 검증
  ...
  - [x] 2.2 `_enrich_aurora_metadata()`, `_get_cluster_info()`, `_INSTANCE_CLASS_MEMORY_MAP` 구현
...
- [x] 3. Checkpoint — Collector enrichment 테스트 통과 확인
  - Ensure all tests pass, ask the user if questions arise.

- [x] 4. Alarm Manager 변형별 알람 라우팅 (`common/alarm_manager.py`)
  - [x] 4.1 테스트 작성: `_get_aurora_alarm_defs()` 6개 변형 검증
  ...
  - [x] 4.2 `_get_aurora_alarm_defs()` 및 신규 알람 정의 상수 구현
...
- [x] 5. Checkpoint — Alarm variant routing 테스트 통과 확인
  - Ensure all tests pass, ask the user if questions arise.

- [x] 6. 퍼센트 기반 FreeableMemory 임계치 (`common/alarm_manager.py`)
  - [x] 6.1 테스트 작성: `_resolve_free_memory_threshold()` 검증
    - `tests/test_alarm_manager.py`에 테스트 추가
  ...
  - [x] 6.2 `_resolve_free_memory_threshold()` 구현
...
- [x] 7. Checkpoint — 퍼센트 임계치 테스트 통과 확인
  - Ensure all tests pass, ask the user if questions arise.

- [x] 8. Aurora 메트릭 수집 조건부 분기 (`common/collectors/rds.py`)
  - [x] 8.1 테스트 작성: `get_aurora_metrics()` 변형별 메트릭 수집 검증
    - `tests/test_collectors.py`에 테스트 추가
  ...
  - [x] 8.2 `get_aurora_metrics()` 조건부 분기 구현
...
- [x] 9. Checkpoint — 메트릭 수집 테스트 통과 확인
  - Ensure all tests pass, ask the user if questions arise.

- [x] 10. Remediation Handler KI-008 수정 (`remediation_handler/lambda_handler.py`)
  - [x] 10.1 테스트 작성: `_handle_delete()` Aurora 삭제 이벤트 알람 정리 검증
  ...
  - [x] 10.2 `_resolve_rds_aurora_type()` 튜플 반환 및 `_handle_delete()` 수정
...
- [x] 11. Checkpoint — KI-008 수정 테스트 통과 확인
  - Ensure all tests pass, ask the user if questions arise.

- [x] 12. Daily Monitor 통합 및 Steering 문서 업데이트
  - [x] 12.1 테스트 작성: `_process_resource()` 신규 메트릭 임계치 비교 검증
  ...
  - [x] 12.2 `_process_resource()` 업데이트 (필요 시)
    - 신규 메트릭 키가 기존 greater-than 비교 로직에 포함되는지 확인
    - `FreeMemoryGB` less-than 비교 집합은 기존 유지 (변경 불필요)
    - _Requirements: 9.1, 10.1, 10.2_

  - [x] 12.3 `.kiro/steering/alarm-rules.md` §12 AuroraRDS 매핑 테이블 업데이트
  ...
  - [x] 12.4 `docs/KNOWN-ISSUES.md` KI-006, KI-007, KI-008 업데이트
