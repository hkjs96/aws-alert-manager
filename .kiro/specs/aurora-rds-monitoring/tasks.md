...
- [x] 2. RDS Collector Aurora 분류 및 메트릭 수집 (`common/collectors/rds.py`)
  - [x] 2.1 테스트 작성: `collect_monitored_resources()` Aurora 분류 검증
  ...
  - [x] 2.2 `collect_monitored_resources()` Aurora 분류 구현
  ...
  - [x] 2.4 테스트 작성: `get_aurora_metrics()` 메트릭 수집 검증
    - `tests/test_collectors.py`에 테스트 추가
...
- [x] 3. Checkpoint — Collector 테스트 통과 확인
  - Ensure all tests pass, ask the user if questions arise.

- [x] 4. Alarm Manager Aurora 알람 정의 (`common/alarm_manager.py`)
  - [x] 4.1 테스트 작성: `_AURORA_RDS_ALARMS` 및 `_get_alarm_defs("AuroraRDS")` 검증
...
- [x] 5. Alarm Manager 알람 검색 호환 (`common/alarm_manager.py`)
  - [x] 5.1 테스트 작성: `_find_alarms_for_resource()` AuroraRDS 검색 검증
    - `tests/test_alarm_manager.py`에 테스트 추가
...
- [x] 6. Checkpoint — Alarm Manager 테스트 통과 확인
  - Ensure all tests pass, ask the user if questions arise.

- [x] 7. Daily Monitor 통합 (`daily_monitor/lambda_handler.py`)
  - [x] 7.1 테스트 작성: `_process_resource()` AuroraRDS 라우팅 검증
  ...
  - [x] 7.2 `_process_resource()` AuroraRDS 라우팅 구현
  ...
  - [x] 7.3 테스트 작성: `_cleanup_orphan_alarms()` AuroraRDS alive_checker 검증
    - `tests/test_daily_monitor.py`에 테스트 추가
    - `alive_checkers["AuroraRDS"]` == `_find_alive_rds_instances` 검증
    - AuroraRDS 알람 → DB 인스턴스 삭제 → 고아 알람 삭제 시나리오 검증
    - _Requirements: 7.1, 7.2, 7.3_

  - [x] 7.4 `_cleanup_orphan_alarms()` alive_checkers 업데이트
...
- [x] 8. Checkpoint — Daily Monitor 테스트 통과 확인
  - Ensure all tests pass, ask the user if questions arise.

- [x] 9. Remediation Handler Aurora 지원 (`remediation_handler/lambda_handler.py`)
  - [x] 9.1 테스트 작성: `_resolve_rds_aurora_type()` 헬퍼 검증
  ...
  - [x] 9.2 `_resolve_rds_aurora_type()` 헬퍼 구현
  ...
  - [x] 9.3 테스트 작성: `parse_cloudtrail_event()` Aurora RDS 이벤트 검증
  ...
  - [x] 9.4 `parse_cloudtrail_event()` Aurora 분기 구현
...
- [x] 10. Tag Resolver AuroraRDS 지원 (`common/tag_resolver.py`)
  - [x] 10.1 테스트 작성: `get_resource_tags("AuroraRDS")` → `_get_rds_tags()` 호출 검증
    - `tests/test_alarm_manager.py` 또는 관련 테스트 파일에 추가
    - _Requirements: 10.1, 10.2, 10.3, 10.4, 10.5_

  - [x] 10.2 `get_resource_tags()` 분기 업데이트
...
- [x] 11. Checkpoint — Remediation + Tag Resolver 테스트 통과 확인
  - Ensure all tests pass, ask the user if questions arise.

- [x] 12. 통합 와이어링 및 §12 매핑 테이블 업데이트
  - [x] 12.1 통합 시나리오 테스트 작성
  ...
  - [x] 12.2 `.kiro/steering/alarm-rules.md` §12 AuroraRDS 매핑 테이블 추가
