...
- [ ] 1. Alarm Registry 데이터 등록 — 12개 리소스 타입 알람 정의 일괄 추가
  ...
  - [ ] 1.2 Green: `common/alarm_registry.py`에 12개 알람 정의 리스트 및 매핑 테이블 추가
...
- [ ] 2. Common Constants 확장 — `common/__init__.py`
  - [ ] 2.1 Red: `tests/test_common_constants.py`에 상수 검증 실패 테스트 작성
...
- [ ] 4. Collectors 구현 — 12개 모듈 (common/collectors/)
  - [ ] 4.1 Red: `tests/test_collectors.py`에 Simple Collector 실패 테스트 작성 — SQS, DynamoDB, EFS, SNS, Route53, DX
  ...
  - [ ] 4.2 Green: Simple Collector 6개 모듈 생성 — `sqs.py`, `dynamodb.py`, `efs.py`, `sns.py`, `route53.py`, `dx.py`
  ...
  - [ ] 4.3 Red: `tests/test_collectors.py`에 Compound Dimension Collector 실패 테스트 작성 — ECS, WAF, S3, SageMaker
  ...
  - [ ] 4.4 Green: Compound Dimension Collector 4개 모듈 생성 — `ecs.py`, `waf.py`, `s3.py`, `sagemaker.py`
  ...
  - [ ] 4.5 Red: `tests/test_collectors.py`에 Special Collector 실패 테스트 작성 — MSK, CloudFront
  ...
  - [ ] 4.6 Green: Special Collector 2개 모듈 생성 — `msk.py`, `cloudfront.py`
  ...
  - [ ] 4.7 Refactor: 12개 Collector 코드 정리 및 전체 테스트 재실행
...
- [ ] 5. Checkpoint — 12개 Collector 구현 완료 확인
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 6. CloudTrail 이벤트 + Remediation Handler — `remediation_handler/lambda_handler.py`
  - [ ] 6.1 Red: `tests/test_remediation_handler.py`에 12개 신규 리소스 이벤트 실패 테스트 작성
  ...
  - [ ] 6.2 Green: Remediation Handler에 12개 신규 리소스 이벤트 매핑 추가
...
- [ ] 7. Tag Resolver 확장 — `common/tag_resolver.py`
  - [ ] 7.1 Red: `tests/test_collectors.py`에 12개 신규 리소스 태그 조회 실패 테스트 작성
  ...
  - [ ] 7.2 Green: `common/tag_resolver.py`의 `get_resource_tags()`에 12개 신규 타입 분기 추가
...
- [ ] 9. Daily Monitor 통합 — `daily_monitor/lambda_handler.py`
  - [ ] 9.1 Red: `tests/test_daily_monitor.py`에 12개 신규 Collector 통합 실패 테스트 작성
  ...
  - [ ] 9.2 Green: Daily Monitor에 12개 신규 Collector 등록
...
- [ ] 13. Alarm Manager 확장 — treat_missing_data + Compound Dimension + region 필드
  - [ ] 13.1 Red: `tests/test_alarm_manager.py`에 treat_missing_data, Compound Dimension, region 실패 테스트 작성
  ...
  - [ ] 13.2 Green: `common/alarm_builder.py` treat_missing_data + region 지원 + `common/dimension_builder.py` Compound Dimension 분기 추가
...
- [ ] 14. Alarm Search 확장 — 신규 리소스 타입 접두사
  - [ ] 14.1 Red: `tests/test_alarm_search.py`에 신규 타입 알람 검색 실패 테스트 작성
  ...
  - [ ] 14.2 Green: `common/alarm_search.py`의 `_find_alarms_for_resource()` 기본 폴백 목록에 12개 신규 타입 추가
...
- [ ] 16. PBT — 설계 문서 Correctness Properties 10개
  ...
  - [ ]* 16.4 PBT Property 4: ECS _ecs_launch_type Alarm Invariance
  ...
  - [ ]* 16.5 PBT Property 5: Compound Dimension Construction (ECS, WAF, S3, SageMaker)
  ...
  - [ ]* 16.6 PBT Property 6: treat_missing_data=breaching for Route53/DX/MSK
  ...
  - [ ]* 16.7 PBT Property 7: metric_name_to_key Round Trip
  ...
  - [ ]* 16.9 PBT Property 9: CloudFront/Route53 us-east-1 Region
  ...
  - [ ]* 16.10 PBT Property 10: Lower-is-Dangerous Threshold Direction Consistency
