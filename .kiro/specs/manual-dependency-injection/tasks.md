...
- [x] 1. `_clients.py`에 `create_clients_for_account` 팩토리 추가
  - [x] 1.1 `create_clients_for_account(role_arn, session_name)` 함수 구현
...
- [x] 2. Checkpoint — 기존 테스트 통과 확인
  - Ensure all tests pass, ask the user if questions arise.

- [x] 3. `alarm_search.py` 함수에 `*, cw=None` 추가
  - [x] 3.1 `_find_alarms_for_resource`, `_delete_all_alarms_for_resource`, `_describe_alarms_batch`에 `*, cw=None` 추가
...
- [x] 4. Checkpoint — 기존 테스트 통과 확인
  - Ensure all tests pass, ask the user if questions arise.

- [x] 5. `dimension_builder.py` 함수에 `*, cw=None` 추가
  - [x] 5.1 `_resolve_metric_dimensions`, `_get_disk_dimensions`에 `*, cw=None` 추가
    - 함수 본문 첫 줄에서 `cw = cw or _clients._get_cw_client()` resolve
...
- [x] 6. Checkpoint — 기존 테스트 통과 확인
  - Ensure all tests pass, ask the user if questions arise.

- [x] 7. `alarm_builder.py`의 `_create_single_alarm`, `_recreate_alarm_by_name`에 `*, cw=None` 추가
  - [x] 7.1 `_create_single_alarm`에 `*, cw=None` 추가
    - 함수 본문 첫 줄에서 `cw = cw or _clients._get_cw_client()` resolve
    - 기존 `cw = _clients._get_cw_client()` 라인을 resolve 패턴으로 교체
    - _Requirements: 3.1, 3.2, 3.5_
  - [x] 7.2 `_recreate_alarm_by_name`에 `*, cw=None` 추가
    - 함수 본문 첫 줄에서 `cw = cw or _clients._get_cw_client()` resolve
    - 기존 `cw = _clients._get_cw_client()` 라인을 resolve 패턴으로 교체
...
- [x] 8. Checkpoint — 기존 테스트 통과 확인
  - Ensure all tests pass, ask the user if questions arise.

- [x] 9. `alarm_sync.py` 함수에 `*, cw=None` 추가
  - [x] 9.1 `_sync_off_hardcoded`, `_sync_dynamic_alarms`, `_apply_sync_changes`에 `*, cw=None` 추가
...
- [x] 10. Checkpoint — 기존 테스트 통과 확인
  - Ensure all tests pass, ask the user if questions arise.

- [x] 11. `alarm_manager.py` Facade 3개 함수에 `*, cw=None` 추가 + 내부 전달
  - [x] 11.1 `create_alarms_for_resource`에 `*, cw=None` 추가
  ...
  - [x] 11.2 `delete_alarms_for_resource`에 `*, cw=None` 추가
    - `_delete_all_alarms_for_resource(resource_id, resource_type, cw=cw)` 전달
    - _Requirements: 1.3, 1.4, 1.7_
  - [x] 11.3 `sync_alarms_for_resource`에 `*, cw=None` 추가
    - `cw = cw or _clients._get_cw_client()` resolve
...
- [x] 12. Checkpoint — 기존 테스트 통과 확인
  - Ensure all tests pass, ask the user if questions arise.

- [x] 13. Property-Based Tests
  - [x]* 13.1 Property 4: keyword-only 파라미터 선언 검증
  ...
  - [x]* 13.2 Property 5: Factory 반환 구조 검증
